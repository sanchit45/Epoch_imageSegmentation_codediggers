...
def Sampling(data,k,centers,Sample_size):				# Coreset construction function.
    ...
    for dt in data:
        ...
        for i,b_i in enumerate(centers):
            ...
            min_d=min(d)
            min_d_idx=d.index(min_d)
            s.append((alpha*dist(data,centers)/c_phi)+((2*alpha*sum([dist(x_i,centers) for x_i in B_i[min_d_idx]]))/(len(B_i[min_d_idx]*c_phi)))+((4*n)/len(B_i[min_d_idx])))
